...
/**
 * Local fork of Fumadocs' `ViewOptionsPopover`
 * (fumadocs-ui/layouts/notebook/page), needed only because that component
 * hardcodes `PopoverContent`'s `align="center"` with no way to override it
 * from the outside. Everything else (item list, icons, links) mirrors the
 * upstream implementation; see:
 * https://fumadocs.dev/docs/integrations/llms#page-actions
 */
export function DocsViewOptionsPopover({
  markdownUrl,
  githubUrl,
  children,
  className,
  ...
  const items = useMemo(() => {
    ...
    ].filter((v): v is Exclude<typeof v, false | undefined | ""> => !!v);
